...
function TypingActivity({ onBack }) {
  ...
  const gameStatus = useMemo(() => {
    ...
    if (generatedBubbles.length > 0 && generatedBubbles.length < totalBubbles && bubbles.length > 0) {
      return 'Em andamento'
    }
  ...
  const handleGameEnd = useCallback(() => {
    ...
    const gameData = {
      ...
      hitRate: generatedBubbles.length > 0 
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (event) => {
      ...
      setGeneratedCharacters(prev => {
        const index = prev.findIndex(char => char.toLowerCase() === pressedKey)
        if (index !== -1) {
      ...
      setBubbles(prev => {
        const index = prev.findIndex(bubble => bubble.character.toLowerCase() === pressedKey)
        if (index !== -1) {
          ...
          if (!hitBubblesRef.current.has(removedBubble.id)) {
            ...
            setHitBubbles(prevHit => {
              ...
              const alreadyExists = prevHit.some(b => b.id === removedBubble.id)
              if (alreadyExists) {
                if (import.meta.env.DEV) {
                  console.warn('Attempt to add duplicate bubble:', removedBubble.id)
                }
                return prevHit
              }
          ...
          .map(bubble => {
            ...
            if (newY < 0) {
              ...
              setGeneratedCharacters(prevChars => {
                const index = prevChars.findIndex(char => char === bubble.character)
                if (index !== -1) {
            ...
            const offsetX = Math.sin(bubble.horizontalOffset + lifetime * frequency * Math.PI * 2) * amplitude
  ...
  useEffect(() => {
    ...
    const loadParams = async () => {
      try {
        ...
        if (!Array.isArray(params?.characters) || params.characters.length === 0) {
        ...
        if (typeof params?.total_bubbles !== 'number' || params.total_bubbles <= 0) {
        ...
        const errorMessage = error.message || 'Não foi possível carregar os parâmetros da atividade'
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center z-10 bg-white/90 backdrop-blur-sm">
              <div className="text-center mb-6">
                ...
                <p className="text-[#777777]">
                  Taxa de acerto: {totalBubbles > 0 ? ((hitBubbles.length / totalBubbles) * 100).toFixed(1) : 0}%
                </p>
              </div>
              <button
                onClick={restartGame}
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <div className="text-xs text-[#777777] space-y-2">
              ...
              <div>
                ...
                <p>Taxa de acerto: {generatedBubbles.length > 0 ? ((hitBubbles.length / generatedBubbles.length) * 100).toFixed(1) : 0}%</p>
              </div>
                  ...
                  {hitBubbles.length === 0 ? (
                    ...
                    <div className="space-y-1">
                      {hitBubbles.map((bubble, index) => (
                        <div key={bubble.id || index} className="text-xs border-b pb-1">
                          <p>ID: {bubble.id?.toString().slice(-6)} | Char: {bubble.character} | Tempo resposta: {bubble.generationTime && bubble.hitTime ? ((bubble.hitTime - bubble.generationTime) / 1000).toFixed(2) : 'N/A'}s</p>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <div className="mt-2">
                <p className="font-semibold mb-1">IDs únicos no Set: {hitBubblesRef.current?.size || 0}</p>
                <p className="text-red-600">Array length: {hitBubbles.length} | Set size: {hitBubblesRef.current?.size || 0}</p>
                {hitBubbles.length !== (hitBubblesRef.current?.size || 0) && (
                  <p className="text-red-600 font-bold">⚠️ ATENÇÃO: Há duplicatas no array!</p>
                )}
                <div className="mt-1 text-xs">
                  <p>IDs no Set: {Array.from(hitBubblesRef.current || []).join(', ').slice(0, 100) || 'Nenhum'}</p>
                </div>
              </div>
            </div>
          </div>
